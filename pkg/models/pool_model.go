...
func NewPoolModel(name string, baseModel string, endpoints []config.EndpointConfig) (*PoolModel, error) {
	...
	for i, ep := range endpoints {
		...
		default:
			// 默认使用自定义模型，支持任意兼容OpenAI API格式的第三方服务
			// 使用baseModel作为实际模型名称，实例名称包含索引后缀
			customModel, err := NewCustomModelWithActualName(
				fmt.Sprintf("%s-%d", name, i), // 实例名称包含索引
				baseModel,                        // 实际模型名称不包含索引
				ep.APIKey,
				ep.URL,
			)
			if err != nil {
				return nil, fmt.Errorf("创建自定义模型失败: %v", err)
			}
			model = customModel
		}
