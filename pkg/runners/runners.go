...
// SetJSONOutput enables or disables JSON formatted output.
func (r *SimpleRunner) SetJSONOutput(enabled bool) {
...
func (r *SimpleRunner) RunInteractive(ctx context.Context, agent *agents.Agent, in io.Reader, out io.Writer) error {
	...
	if !r.jsonOutput {
		// Print welcome message only in non-JSON mode
	...
	for {
		// Prompt only if not in JSON mode
		if !r.jsonOutput {
			fmt.Fprintf(out, "> ")
		}
	...
	// Save the session if enabled (only non-JSON mode displays message)
	if r.saveSession {
		// in JSON mode do not print message, still save file
